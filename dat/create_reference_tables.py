...
# create the env with this command: conda env create -f dat/envs/pyspark-322-delta-200
...
def create_reference_table1():
    ...
    df.write.format("delta").save("dat/reference_tables/reference_table1/table")
    ...
    df.write.mode("append").format("delta").save(
        "dat/reference_tables/reference_table1/table"
    )

    df = spark.read.format("delta").load("dat/reference_tables/reference_table1/table")

    df.toPandas().to_parquet(
        "dat/reference_tables/reference_table1/table_content.parquet"
    )
...
def create_reference_table2():
    ...
    df.write.partitionBy("letter").format("delta").save(
        "dat/reference_tables/reference_table2/table"
    )
    ...
    df = spark.read.format("delta").load("dat/reference_tables/reference_table2/table")

    df.toPandas().to_parquet(
        "dat/reference_tables/reference_table2/table_content.parquet"
    )
